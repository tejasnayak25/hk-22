...
import { loginWithGoogle, loginRequired, auth, loginWithEmail } from "../fbase-client";
                ...
                <div className="flex justify-center md:items-center items-start flex-1 md:w-1/2 w-full p-8">
                    <div className="w-full md:max-w-md">
                        <div className="h-full border-2 border-[#00bcae] bg-[#003a35] rounded-lg overflow-hidden p-6">
                            <Form items={[{ type: "input", id: "email", ref: emailRef, label: "Email", placeholder: "abc@gmail.com", inp_type: "text" }, { type: "input", id: "password", label: "Password", ref: passwordRef, placeholder: "secret", inp_type: "password" }, { type: "link", href: "/forgot-password", label: "Forgot Password?" }, { type: "button", label: "Login",
                                onClick: async () => {
                                    if(emailRef.current && passwordRef.current) {
                                        ...
                                        let user = await loginWithEmail(email, password, "login");
                                        if(user) {
                                            router.push("/");
                                        }
                                    }
